      ...
      | .const fn us =>
        ...
        | none =>
          if TransforM.isBuiltin fn then
            ...
            let fn ← mkAuxiliaryMatcher fn params motive
            let args := args.drop (info.numParams + 1)
            elimExprRaw' (mkAppN fn args) inProp subst
          else if isCasesOnRecursor (← getEnv) fn then
            let .str indName _ := fn | throwError s!"Weird casesOn: {fn}"
            let indInfo ← getConstInfoInduct indName
            let params := args[0...indInfo.numParams].toArray
            let motive := args[indInfo.numParams]!
            let fn ← mkAuxiliaryCasesOn fn params motive indInfo
            let args := args.drop (indInfo.numParams + 1)
            elimExprRaw' (mkAppN fn args) inProp subst
          else
...
public def elim (g : MVarId) : DepM MVarId := do
